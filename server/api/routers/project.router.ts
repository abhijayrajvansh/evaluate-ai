...
export const projectRouter = createTRPCRouter({
  eslintCheck: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      execSync(
        `docker run -d --name ${containerName} -v /tmp:/tmp eslint-runner`
      );
      ...
      try {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          containerStatus = execSync(`docker inspect -f '{{.State.Status}}' ${containerName}`)
            .toString()
            .trim();
          ...
          console.log(`Attempt ${attempt + 1}: Container status is '${containerStatus}'. Retrying in ${retryInterval}ms...`);
          await new Promise(resolve => setTimeout(resolve, retryInterval));
        }

        if (containerStatus !== 'running') {
          throw new Error(`Container ${containerName} failed to start. Final status: ${containerStatus}`);
        }
        ...
        `;
        execSync(`docker exec ${containerName} bash -c "cd ${workspacePath} && ${cloneCommand}"`);

        console.log(`Running ESLint on the files`);
        const eslintOutput = execSync(`docker exec ${containerName} bash -c "eslint --format json ${workspacePath}"`)
          .toString();
